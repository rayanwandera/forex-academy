      ...
      <div style={{ display: 'flex', gap: 24, margin: '1.5rem 0' }}>
        ...
        <div className="card" style={{ flex: 1 }}>
          <div className="mono" style={{ fontSize: '1.6rem' }}>
            {referrals.filter((r) => r.status === 'pending').reduce((s, r) => s + r.amount_ugx, 0).toLocaleString()} UGX
          </div>
          <div style={{ fontSize: '0.85rem', color: 'var(--text-dim)' }}>Referral payouts owed</div>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {['payments', 'referrals', 'users'].map((t) => (
